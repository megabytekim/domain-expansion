...
- [x] **필터 슬라이더 초기 상태에 데이터 범위 노출** ✅ 2026-05-21 — `priceMin`/`durationMin` 빌드 타임 계산 후 라벨에 fallback
- [~] **출발 일정 테이블 "예약 > 정원" 표기** — 전체 1413개 중 8개(0.6%), 모두 잔여=0. 임팩트 작아 보류 (2026-05-21 결정)
- [~] **검색 결과 8개 하드캡** — 우상단에 매칭 카운트 이미 노출 (resultCount), 보류 (2026-05-21 결정)
- [~] **모바일 상단 영역 점유율** — 보류 (2026-05-21 결정)
- [~] **마커 색상 의미 부여** — 논의만 저장 (2026-05-21):
    - 현재: `productId % 50` 팔레트 → 무의미한 색
    - v1 안: 카테고리 4색 매핑 (트레킹 녹/문화 노랑/도보 파랑/기획 핑크) → 분포 한눈에 보이나, 카테고리당 ~40개 패키지 동시 구분은 색만으론 불가
    - v2 안 (v1 위에): 마커 hover 시 `locations[0] → [N-1]` 일정 순서 polyline 표시 (데이터에 순서 보존됨, 안나푸르나·나카센도 등 실제 여행 순서). 살짝 굽은 arc + 방향 화살표
    - 보류 사유: 코드 복잡도 (line layer + hover 동적 GeoJSON 갱신)
- [~] **마커 hit-radius** — 보류 (2026-05-21 결정)
- [~] **UNESCO 데이터 활용 확인** — 1245개 사이트 GeoJSON + 15개 매핑 존재. 현재 UI 미노출. 보류 (2026-05-21 결정)

...
[제외]
- 한국 출발/도착 공항 (인천, 김포)
- 단순 경유 도시 — 다음 패턴은 모두 환승 공항으로 간주하고 제외:
  - 괄호 안 "(경유)" 표시
  - 항공사 코드 (TK)/(EK)/(OZ)/(KE)/(AF)/(LH) 등이 도시 뒤 또는 패키지명에 붙은 경우
  - **도시명(공항코드) 패턴**: "파리(CDG)", "런던(LHR)", "두바이(DXB)", "이스탄불(IST)" 등 — IATA 3자리 공항코드가 괄호로 붙어있으면 일정상 머무는 게 아니라 환승. 출국/귀국 항공편 일정에 등장하는 것은 모두 제외.
